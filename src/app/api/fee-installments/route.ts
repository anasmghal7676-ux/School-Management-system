...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.entity === 'plan') {
      ...
      await prisma.systemSetting.create({ data: { key: PLAN_KEY + id, value: JSON.stringify(item) } });
    ...
    for (let i = 0; i < (Number(installments) || 0); i++) {
      ...
      await prisma.systemSetting.create({ data: { key: PAY_KEY + pid, value: JSON.stringify(payment) } });
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await prisma.systemSetting.delete({ where: { key: (entity === 'plan' ? PLAN_KEY : PAY_KEY) + id } });
