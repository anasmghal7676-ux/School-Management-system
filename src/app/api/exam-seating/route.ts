...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.action === 'auto_assign') {
      ...
      const hall = JSON.parse((await db.systemSetting.findUnique({ where: { schoolId_settingKey: { schoolId: 'school_main', settingKey: HALL_KEY + hallId } } }))?.value || '{}');
      ...
      for (let i = 0; i < Math.min(students.length, maxSeats); i++) {
        ...
        await db.systemSetting.create({ data: { settingKey: SEAT_KEY + seatId, settingValue: JSON.stringify(seat), schoolId: 'school_main', settingType: 'General' } });
        created.push(seat);
      }
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (entity === 'all_seats') {
      ...
      await Promise.all(toDelete.map((s: any) => db.systemSetting.delete({ where: { schoolId_settingKey: { schoolId: 'school_main', settingKey: SEAT_KEY + s.id } } })));
