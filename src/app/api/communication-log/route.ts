...
export async function POST(req: NextRequest) {
  try {
    ...
    await db.systemSetting.create({
      data: { settingKey: `comm_log_entry_${id}`, settingValue: JSON.stringify(log), schoolId: process.env.SCHOOL_ID || 'school_main', settingType: 'General' },
    });
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const setting = await db.systemSetting.findUnique({ where: { schoolId_settingKey: { schoolId: process.env.SCHOOL_ID || 'school_main', settingKey: `comm_log_entry_${id}` } } });
    if (!setting) return NextResponse.json({ error: 'Not found' }, { status: 404 });
    const existing = JSON.parse(setting.settingValue);
    const updated = { ...existing, ...updates };
    await db.systemSetting.update({
      where: { schoolId_settingKey: { schoolId: process.env.SCHOOL_ID || 'school_main', settingKey: `comm_log_entry_${id}` } }, data: { settingValue: JSON.stringify(updated) },
    });
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await db.systemSetting.delete({ where: { schoolId_settingKey: { schoolId: process.env.SCHOOL_ID || 'school_main', settingKey: `comm_log_entry_${id}` } } });
