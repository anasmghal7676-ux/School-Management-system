...
export async function GET(req: NextRequest) {
  try {
    ...
    if (view === 'submissions') {
      ...
      const students = await prisma.student.findMany({ where: { status: 'Active', ...(classId ? { classId } : {}) }, select: { id: true, fullName: true, admissionNumber: true, rollNumber: true, class: { select: { name: true } } }, orderBy: [{ rollNumber: 'asc' }, { fullName: 'asc' }] });
