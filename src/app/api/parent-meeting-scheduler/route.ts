...
export async function GET(req: NextRequest) {
  try {
    ...
    if (view === 'bookings') {
      ...
      const students = await prisma.student.findMany({ where: { status: 'Active' }, select: { id: true, fullName: true, admissionNumber: true, class: { select: { name: true, id: true } }, fatherName: true, fatherPhone: true }, orderBy: { fullName: 'asc' } });
