...
const KEY = 'vehicle-maintenance_entry_';
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (search) { const s = search.toLowerCase(); items = items.filter((i: any) => JSON.stringify(i).toLowerCase().includes(s)); }
    if (filter) items = items.filter((i: any) => i.category === filter || i.type === filter || i.status === filter);
    items.sort((a: any, b: any) => new Date(b.createdAt || b.date || 0).getTime() - new Date(a.createdAt || a.date || 0).getTime());
    const total = items.length;
    return NextResponse.json({ items: items.slice((page-1)*limit, page*limit), total });
