...
export async function GET(req: NextRequest) {
  try {
    ...
    const summary = {
      ...
      totalClaimed: items.reduce((s: number, i: any) => s + Number(i.amount || 0), 0),
      totalApproved: items.filter((i: any) => i.status !== 'Rejected').reduce((s: number, i: any) => s + Number(i.approvedAmount || i.amount || 0), 0),
    };
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const prev = JSON.parse(s.settingValue);
    const updated = { ...prev, ...updates, updatedAt: new Date().toISOString() };
    if (['Approved', 'Rejected', 'Paid'].includes(updates.status)) updated.reviewedAt = new Date().toISOString();
