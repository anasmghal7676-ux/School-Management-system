...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.action === 'save_value') {
      ...
      await db.systemSetting.upsert({ where: { schoolId_settingKey: { schoolId: process.env.SCHOOL_ID || 'school_main', settingKey: vKey } }, create: { settingKey: vKey, settingValue: JSON.stringify(item), schoolId: process.env.SCHOOL_ID || 'school_main', settingType: 'General' }, update: { settingValue: JSON.stringify(item) } });
