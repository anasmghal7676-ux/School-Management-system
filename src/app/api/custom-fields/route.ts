...
async function getByPrefix(prefix: string) {
  const s = await db.systemSetting.findMany({ where: { key: { startsWith: prefix } } });
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.action === 'save_value') {
      ...
      await db.systemSetting.upsert({ where: { key: vKey }, create: { key: vKey, value: JSON.stringify(item) }, update: { value: JSON.stringify(item) } });
