...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.entity === 'venue') {
      ...
      await db.systemSetting.create({ data: { settingKey: VENUE_KEY + id, settingValue: JSON.stringify(venue), schoolId: process.env.SCHOOL_ID || 'school_main', settingType: 'General' } });
    ...
    await db.systemSetting.create({ data: { settingKey: BOOKING_KEY + id, settingValue: JSON.stringify(booking), schoolId: process.env.SCHOOL_ID || 'school_main', settingType: 'General' } });
