...
export default function FinReportsPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/fin-reports/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Report', 'Period', 'Income', 'Expense', 'Balance'];
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No financial reports found</Text></Center>
                  </Table.Td>
                </Table.Tr>
