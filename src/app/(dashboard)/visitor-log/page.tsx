...
export default function VisitorLogPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/visitor-log/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Name', 'Purpose', 'Host', 'Entry', 'Exit', 'Status'];
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No visitor log records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
