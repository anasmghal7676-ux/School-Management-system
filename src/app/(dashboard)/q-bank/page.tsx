...
export default function QBankPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/q-bank/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Question', 'Subject', 'Class', 'Type', 'Difficulty'];
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No question bank records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
