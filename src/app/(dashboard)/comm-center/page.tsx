...
export default function CommCenterPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/comm-center?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/comm-center/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Message', 'Channel', 'Recipients', 'Sent By', 'Date'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Communication Center</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No communication center found</Text></Center>
                  </Table.Td>
                </Table.Tr>
