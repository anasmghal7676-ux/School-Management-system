...
export default function ExitMgmtPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/exit-mgmt/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Exit Management</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No exit management records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
