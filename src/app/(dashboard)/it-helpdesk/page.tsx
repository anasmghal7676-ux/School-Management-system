...
export default function ItHelpdeskPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/it-helpdesk?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/it-helpdesk/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Ticket No', 'Issue', 'Reported By', 'Priority', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>IT Helpdesk</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No it helpdesk records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
