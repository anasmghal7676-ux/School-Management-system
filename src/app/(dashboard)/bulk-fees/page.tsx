...
export default function BulkFeesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/bulk-fees?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/bulk-fees/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Class', 'Fee Type', 'Amount', 'Posted By', 'Date'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Bulk Fee Posting</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No bulk fee posting found</Text></Center>
                  </Table.Td>
                </Table.Tr>
