...
export default function AcadPlannerPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/acad-planner/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Academic Planner</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No academic planner found</Text></Center>
                  </Table.Td>
                </Table.Tr>
