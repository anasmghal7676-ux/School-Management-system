...
export default function CustFieldsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/cust-fields?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/cust-fields/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Label', 'Module', 'Type', 'Required', 'Status'];
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No custom fields records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
