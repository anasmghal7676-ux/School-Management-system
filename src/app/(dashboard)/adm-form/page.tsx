...
export default function AdmFormPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/adm-form/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Applicant', 'Class', 'Applied', 'Score', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Admission Forms</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No admission forms found</Text></Center>
                  </Table.Td>
                </Table.Tr>
