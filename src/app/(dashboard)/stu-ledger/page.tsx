...
export default function StuLedgerPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/stu-ledger/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Student', 'Date', 'Description', 'Debit', 'Credit', 'Balance'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Student Ledger</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No student ledger records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
