...
export default function CctvLogPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/cctv-log/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Camera', 'Location', 'Event', 'Operator', 'Date'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>CCTV Logs</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No cctv logs records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
