...
export default function TcIssuancePage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/tc-issuance/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Student', 'Issued Date', 'Reason', 'Issued By', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>TC Issuance</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No tc issuance records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
