...
export default function TeachPortalPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/teach-portal?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/teach-portal/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Teacher', 'Last Login', 'Classes', 'Marks Entered', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Teacher Portal</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No teacher portal records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
