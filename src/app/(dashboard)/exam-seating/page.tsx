...
export default function ExamSeatingPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/exam-seating/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Student', 'Exam', 'Hall', 'Seat No', 'Date'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Exam Seating</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No exam seating found</Text></Center>
                  </Table.Td>
                </Table.Tr>
