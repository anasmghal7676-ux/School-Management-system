...
export default function AcadSessionPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/acad-sessions?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/acad-sessions/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Name', 'Start Date', 'End Date', 'Current', 'Status'];
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No academic sessions found</Text></Center>
                  </Table.Td>
                </Table.Tr>
