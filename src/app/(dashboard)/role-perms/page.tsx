...
export default function RolePermsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/role-perms?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/role-perms/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Role', 'Module', 'Read', 'Write', 'Delete'];
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No role permissions records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
