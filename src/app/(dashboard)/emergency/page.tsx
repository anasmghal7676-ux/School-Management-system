...
export default function EmergencyPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/emergency/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Student', 'Contact Name', 'Relation', 'Phone', 'Type'];
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No emergency contacts records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
