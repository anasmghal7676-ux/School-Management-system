...
export default function NotifRulesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/notif-rules?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/notif-rules/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Rule Name', 'Trigger', 'Channel', 'Recipients', 'Status'];
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No notification rules records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
