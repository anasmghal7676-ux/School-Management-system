...
export default function BehaviorPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/behavior/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Student', 'Incident', 'Date', 'Action', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Behavior Records</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No behavior records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
