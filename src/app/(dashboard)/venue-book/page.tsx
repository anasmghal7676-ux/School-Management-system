...
export default function VenueBookPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/venue-book/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Venue', 'Event', 'Date/Time', 'Booked By', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Venue Booking</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No venue booking records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
