...
export default function BulkImportPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/bulk-import?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/bulk-import/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['File', 'Module', 'Total', 'Success', 'Errors', 'Date'];
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No bulk import records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
