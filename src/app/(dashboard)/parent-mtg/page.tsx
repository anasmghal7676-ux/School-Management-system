...
export default function ParentMtgPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/parent-mtg/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Parent', 'Teacher', 'Student', 'Date/Time', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Parent Meetings</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No parent meetings records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
