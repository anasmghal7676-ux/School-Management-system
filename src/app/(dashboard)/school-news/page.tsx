...
export default function SchoolNewsPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/school-news/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Title', 'Category', 'Author', 'Published', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>School News</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No school news records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
