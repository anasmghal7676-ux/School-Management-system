...
export default function ExamControlPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/exam-control/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Exam', 'Class', 'Date', 'Venue', 'Supervisor'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Exam Control</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No exam control found</Text></Center>
                  </Table.Td>
                </Table.Tr>
