...
export default function RptBuilderPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/rpt-builder?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/rpt-builder/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Report Name', 'Module', 'Filters', 'Created By', 'Date'];
        ...
        {stats.map(s => (
          <Grid.Col key={s.label} span={{ base: 12, sm: 4 }}>
            <Card withBorder radius="md" p="md">
              <Text size="xl" fw={700} c="grape">{}</Text>
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No report builder records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
