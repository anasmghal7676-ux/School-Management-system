...
export default function ClearancePage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/clearance?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/clearance/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Name', 'Type', 'Reason', 'Date', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Clearance</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No clearance found</Text></Center>
                  </Table.Td>
                </Table.Tr>
