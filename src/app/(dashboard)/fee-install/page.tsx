...
export default function FeeInstallPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/fee-install/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Student', 'Plan', 'Installments', 'Paid', 'Balance'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Fee Installments</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No fee installments found</Text></Center>
                  </Table.Td>
                </Table.Tr>
