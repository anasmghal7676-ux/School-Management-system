...
export default function SysHealthPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/sys-health?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/sys-health/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Service', 'Status', 'Uptime', 'Response Time', 'Last Check'];
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No system health records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
