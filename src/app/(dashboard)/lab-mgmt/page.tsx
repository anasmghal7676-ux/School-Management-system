...
export default function LabMgmtPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/lab-mgmt/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Lab', 'Subject', 'Teacher', 'Date/Time', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Lab Management</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No lab management records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
