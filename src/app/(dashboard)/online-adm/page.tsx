...
export default function OnlineAdmPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/online-adm/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Applicant', 'Program', 'Applied Date', 'Score', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Online Admissions</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No online admissions records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
