...
export default function ClsTimetablePage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/cls-timetable?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/cls-timetable/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Class', 'Day', 'Period', 'Subject', 'Teacher'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Class Timetable</Title>
        <Group gap="sm">
        ...
        {[
          ...
          <Grid.Col key={s.label} span={{ base: 12, sm: 4 }}>
            <Card withBorder radius="md" p="md">
              <Text size="xl" fw={700} c="indigo">{s.value}</Text>
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No class timetable found</Text></Center>
                  </Table.Td>
                </Table.Tr>
