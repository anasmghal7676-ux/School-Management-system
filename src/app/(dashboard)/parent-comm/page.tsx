...
export default function ParentCommPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/parent-comm?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/parent-comm/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Parent', 'Student', 'Subject', 'Channel', 'Date'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Parent Communication</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No parent communication records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
