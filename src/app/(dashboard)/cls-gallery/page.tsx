...
export default function ClsGalleryPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/cls-gallery?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/cls-gallery/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Title', 'Class', 'Photos', 'Uploaded By', 'Date'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Class Gallery</Title>
        <Group gap="sm">
        ...
        {[
          ...
          <Grid.Col key={s.label} span={{ base: 12, sm: 4 }}>
            <Card withBorder radius="md" p="md">
              <Text size="xl" fw={700} c="cyan">{s.value}</Text>
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No class gallery found</Text></Center>
                  </Table.Td>
                </Table.Tr>
