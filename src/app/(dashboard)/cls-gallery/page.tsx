...
import { IconRefresh, IconAlertCircle, IconPhoto } from '@tabler/icons-react';
...
export default function Page() {
  ...
  async function load() {
    setLoading(true);
    try {
      const r = await fetch('/api/class-photo-gallery').then(r => r.json());
  ...
  return (
    <Box p="xl">
      <Group justify="space-between" mb="xl">
        <Box>
          <Text size="xl" fw={700} c="#0f172a">Class Gallery</Text>
          <Text size="sm" c="dimmed">Class photos & media</Text>
        </Box>
      ...
      <Card shadow="xs" radius="md" p="xl" style={{ border: '1px solid #f1f5f9', textAlign: 'center' }}>
        {loading ? (
          ...
          <Stack align="center" gap="md" py="xl">
            <Alert icon={<IconAlertCircle size={16} />} color="pink" radius="md" style={{ maxWidth: 400 }}>
              No class gallery records found. This section will populate as you use the system.
            </Alert>
          </Stack>
        ) : (
          <Stack align="center" gap="sm" py="md">
            <Badge size="xl" variant="light" color="pink">{data.length} records loaded</Badge>
