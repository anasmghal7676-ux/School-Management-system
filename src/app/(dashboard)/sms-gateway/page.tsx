...
export default function SmsGatewayPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/sms-gateway?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/sms-gateway/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Recipient', 'Message', 'Gateway', 'Status', 'Date'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>SMS Gateway</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No sms gateway records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
