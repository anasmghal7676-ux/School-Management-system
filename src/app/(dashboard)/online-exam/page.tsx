...
export default function OnlineExamPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/online-exam?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/online-exam/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Title', 'Subject', 'Class', 'Duration', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Online Exams</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No online exams records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
