...
export default function StaffXferPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/staff-xfer?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/staff-xfer/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Staff', 'From Dept', 'To Dept', 'Effective', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Staff Transfers</Title>
        <Group gap="sm">
        ...
        {stats.map(s => (
          <Grid.Col key={s.label} span={{ base: 12, sm: 4 }}>
            <Card withBorder radius="md" p="md">
              <Text size="xl" fw={700} c="cyan">{}</Text>
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No staff transfers records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
