...
export default function EventsMgmtPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/events-mgmt?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/events-mgmt/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Event', 'Date', 'Venue', 'Organizer', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Events Management</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No events management found</Text></Center>
                  </Table.Td>
                </Table.Tr>
