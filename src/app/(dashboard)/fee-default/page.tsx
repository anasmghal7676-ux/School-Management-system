...
export default function FeeDefaultPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/fee-default?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/fee-default/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Student', 'Class', 'Amount Due', 'Due Since', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Fee Defaulters</Title>
        <Group gap="sm">
        ...
        {[
          ...
          <Grid.Col key={s.label} span={{ base: 12, sm: 4 }}>
            <Card withBorder radius="md" p="md">
              <Text size="xl" fw={700} c="red">{s.value}</Text>
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No fee defaulters found</Text></Center>
                  </Table.Td>
                </Table.Tr>
