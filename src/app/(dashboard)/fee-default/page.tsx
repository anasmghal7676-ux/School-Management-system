...
export default function Page() {
  ...
  async function load() {
    setLoading(true);
    try {
      const r = await fetch('/api/fee-defaults').then(r => r.json());
  ...
  return (
    <Box p="xl">
      <Group justify="space-between" mb="xl">
        <Box>
          <Text size="xl" fw={700} c="#0f172a">Fee Defaults</Text>
          <Text size="sm" c="dimmed">Default fee settings per class</Text>
        </Box>
        ...
        {loading ? (
          ...
          <Stack align="center" gap="md" py="xl">
            <Alert icon={<IconAlertCircle size={16} />} color="blue" radius="md" style={{ maxWidth: 400 }}>
              No fee defaults records found. This section will populate as you use the system.
            </Alert>
          </Stack>
        ) : (
