...
export default function Page() {
  ...
  async function load() {
    setLoading(true);
    try {
      const r = await fetch('/api/communication-log').then(r => r.json());
  ...
  return (
    <Box p="xl">
      <Group justify="space-between" mb="xl">
        <Box><Text size="xl" fw={700} c="#0f172a">Communication Log</Text><Text size="sm" c="dimmed">All sent messages history</Text></Box>
      ...
      <Card shadow="xs" radius="md" p="xl" style={{ border: '1px solid #f1f5f9' }}>
        {loading ? <Group justify="center" py="xl"><Loader /></Group> : data.length === 0 ? (
          <Alert icon={<IconAlertCircle size={16} />} color="blue" radius="md">No communication log records found. Data will appear here once added.</Alert>
        ) : (
          <Table striped highlightOnHover>
            <Table.Thead><Table.Tr><Table.Th>Type</Table.Th><Table.Th>Recipient</Table.Th><Table.Th>Subject</Table.Th><Table.Th>Date</Table.Th><Table.Th>Date</Table.Th></Table.Tr></Table.Thead>
            <Table.Tbody>
