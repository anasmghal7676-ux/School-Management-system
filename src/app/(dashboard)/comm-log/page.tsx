...
export default function CommLogPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/comm-log/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Type', 'Recipient', 'Subject', 'Status', 'Date'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Communication Log</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No communication log records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
