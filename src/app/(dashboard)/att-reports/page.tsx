...
export default function AttReportsPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/att-reports/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Student/Staff', 'Month', 'Present', 'Absent', '%'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Attendance Reports</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No attendance reports found</Text></Center>
                  </Table.Td>
                </Table.Tr>
