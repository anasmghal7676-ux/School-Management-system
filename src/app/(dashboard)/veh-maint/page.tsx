...
export default function VehMaintPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/veh-maint?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/veh-maint/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Vehicle', 'Service Type', 'Date', 'Cost', 'Next Due'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Vehicle Maintenance</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No vehicle maintenance records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
