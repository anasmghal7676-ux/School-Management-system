...
export default function HwTrackerPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/hw-tracker/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Subject', 'Teacher', 'Class', 'Due Date', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Homework Tracker</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No homework tracker found</Text></Center>
                  </Table.Td>
                </Table.Tr>
