...
export default function ExpenseReqPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/expense-req?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/expense-req/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Staff', 'Category', 'Amount', 'Date', 'Status'];
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No expense requests found</Text></Center>
                  </Table.Td>
                </Table.Tr>
