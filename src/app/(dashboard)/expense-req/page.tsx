...
import { IconRefresh, IconAlertCircle, IconReceipt } from '@tabler/icons-react';
...
export default function Page() {
  ...
  async function load() {
    setLoading(true);
    try {
      const r = await fetch('/api/expense-requests').then(r => r.json());
  ...
  return (
    <Box p="xl">
      <Group justify="space-between" mb="xl">
        <Box>
          <Text size="xl" fw={700} c="#0f172a">Expense Requests</Text>
          <Text size="sm" c="dimmed">Staff expense requests</Text>
        </Box>
        ...
        {loading ? (
          ...
          <Stack align="center" gap="md" py="xl">
            <Alert icon={<IconAlertCircle size={16} />} color="orange" radius="md" style={{ maxWidth: 400 }}>
              No expense requests records found. This section will populate as you use the system.
            </Alert>
          </Stack>
        ) : (
