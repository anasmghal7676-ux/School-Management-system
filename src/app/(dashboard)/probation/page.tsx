...
export default function ProbationPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/probation/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Staff', 'Start Date', 'End Date', 'Supervisor', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Probation Records</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No probation records records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
