...
export default function ParentPPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/parent-p?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/parent-p/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Parent', 'Student', 'Last Login', 'Actions', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Parent Portal</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No parent portal records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
