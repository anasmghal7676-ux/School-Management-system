...
export default function NoticeBoardPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/notice-board?' + params);
  ...
  const cols  = ['Title', 'Category', 'Posted By', 'Expiry', 'Status'];
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No notice board found</Text></Center>
                  </Table.Td>
                </Table.Tr>
