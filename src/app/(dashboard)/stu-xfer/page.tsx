...
export default function StuXferPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/stu-xfer/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Student', 'From School', 'To School', 'Date', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Student Transfers</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No student transfers records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
