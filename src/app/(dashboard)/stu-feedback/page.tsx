...
export default function StuFeedbackPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/stu-feedback?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/stu-feedback/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Student', 'Subject', 'Rating', 'Comment', 'Date'];
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No student feedback records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
