...
export default function MsgTmplPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/msg-tmpl/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Title', 'Type', 'Language', 'Variables', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Message Templates</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No message templates records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
