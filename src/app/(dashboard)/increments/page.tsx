...
export default function IncrementsPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/increments/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Staff', 'Previous', 'New Salary', 'Effective', 'Approved By'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Salary Increments</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No salary increments records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
