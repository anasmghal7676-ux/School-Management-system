...
export default function InvReqPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/inv-req?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/inv-req/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Item', 'Quantity', 'Requested By', 'Date', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Inventory Requests</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No inventory requests found</Text></Center>
                  </Table.Td>
                </Table.Tr>
