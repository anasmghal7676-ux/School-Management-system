...
export default function AssetsTrackPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/assets-track?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/assets-track/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Asset', 'Location', 'Assigned To', 'Condition', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Asset Tracking</Title>
        <Group gap="sm">
        ...
        {[
          ...
          <Grid.Col key={s.label} span={{ base: 12, sm: 4 }}>
            <Card withBorder radius="md" p="md">
              <Text size="xl" fw={700} c="orange">{s.value}</Text>
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No asset tracking found</Text></Center>
                  </Table.Td>
                </Table.Tr>
