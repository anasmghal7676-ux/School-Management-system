...
export default function CmpReportPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/cmp-report?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/cmp-report/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Metric', 'Last Year', 'This Year', 'Change', 'Trend'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Comparative Report</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No comparative report found</Text></Center>
                  </Table.Td>
                </Table.Tr>
