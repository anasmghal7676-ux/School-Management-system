...
export default function ExamTtPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/exam-tt/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Exam', 'Subject', 'Class', 'Date/Time', 'Venue'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Exam Timetable</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No exam timetable found</Text></Center>
                  </Table.Td>
                </Table.Tr>
