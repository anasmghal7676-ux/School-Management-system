...
export default function IdCardsPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/id-cards/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Name', 'Type', 'Class/Dept', 'Issued Date', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>ID Card Printing</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No id card printing records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
