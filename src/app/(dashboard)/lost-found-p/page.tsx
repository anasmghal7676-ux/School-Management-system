...
export default function LostFoundPPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/lost-found-p/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Item', 'Found By', 'Location', 'Date', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Lost & Found</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No lost & found records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
