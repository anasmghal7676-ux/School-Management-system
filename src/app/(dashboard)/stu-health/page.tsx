...
export default function Page() {
  ...
  async function load() {
    setLoading(true);
    try {
      const r = await fetch('/api/student-health').then(r => r.json());
  ...
  return (
    <Box p="xl">
      <Group justify="space-between" mb="xl">
        <Box><Text size="xl" fw={700} c="#0f172a">Student Health</Text><Text size="sm" c="dimmed">Health records & medical info</Text></Box>
      ...
      <Card shadow="xs" radius="md" p="xl" style={{ border: '1px solid #f1f5f9' }}>
        {loading ? <Group justify="center" py="xl"><Loader /></Group> : data.length === 0 ? (
          <Alert icon={<IconAlertCircle size={16} />} color="blue" radius="md">No student health records found. Data will appear here once added.</Alert>
        ) : (
          <Table striped highlightOnHover>
            <Table.Thead><Table.Tr><Table.Th>Student</Table.Th><Table.Th>Condition</Table.Th><Table.Th>Blood Group</Table.Th><Table.Th>Emergency Contact</Table.Th><Table.Th>Date</Table.Th></Table.Tr></Table.Thead>
            <Table.Tbody>
              {data.slice(0, 50).map((r: any, i: number) => (
                <Table.Tr key={r.id || i}>
                  <Table.Td><Text size="sm">{String((r as any)[Object.keys(r as any)[0]] ?? '—')}</Text></Table.Td><Table.Td><Text size="sm">{String((r as any)[Object.keys(r as any)[1]] ?? '—')}</Text></Table.Td><Table.Td><Text size="sm">{String((r as any)[Object.keys(r as any)[2]] ?? '—')}</Text></Table.Td><Table.Td><Text size="sm">{String((r as any)[Object.keys(r as any)[3]] ?? '—')}</Text></Table.Td>
