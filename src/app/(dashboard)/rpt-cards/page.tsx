...
export default function RptCardsPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/rpt-cards/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Student', 'Class', 'Term', 'Score', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Report Cards (Alt)</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No report cards (alt) records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
