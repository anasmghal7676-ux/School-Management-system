...
export default function StaffLoadPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/staff-load?' + params);
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/staff-load/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Staff', 'Periods/Week', 'Classes', 'Subjects', 'Load %'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Staff Workload</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No staff workload records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
