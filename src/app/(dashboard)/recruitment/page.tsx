...
export default function Page() {
  ...
  async function load() {
    setLoading(true);
    try {
      const r = await fetch('/api/recruitment').then(r => r.json());
  ...
  return (
    <Box p="xl">
      <Group justify="space-between" mb="xl">
        <Box><Text size="xl" fw={700} c="#0f172a">Recruitment</Text><Text size="sm" c="dimmed">Staff hiring pipeline</Text></Box>
      ...
      <Card shadow="xs" radius="md" p="xl" style={{ border: '1px solid #f1f5f9' }}>
        {loading ? <Group justify="center" py="xl"><Loader /></Group> : data.length === 0 ? (
          <Alert icon={<IconAlertCircle size={16} />} color="blue" radius="md">No recruitment records found. Data will appear here once added.</Alert>
        ) : (
          <Table striped highlightOnHover>
            <Table.Thead><Table.Tr><Table.Th>Position</Table.Th><Table.Th>Applicant</Table.Th><Table.Th>Applied</Table.Th><Table.Th>Stage</Table.Th><Table.Th>Status</Table.Th><Table.Th>Date</Table.Th></Table.Tr></Table.Thead>
            <Table.Tbody>
