...
export default function FeeRemindPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/fee-remind/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Student', 'Channel', 'Amount', 'Sent At', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Fee Reminders</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No fee reminders found</Text></Center>
                  </Table.Td>
                </Table.Tr>
