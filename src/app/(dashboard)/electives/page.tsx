...
export default function ElectivesPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res  = await fetch(`/api/electives/${id}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.success) {
  ...
  const cols  = ['Subject', 'Class', 'Teacher', 'Students', 'Status'];

  return (
    <Container size="xl" py="md">
      <Group justify="space-between" mb="lg">
        <Title order={2}>Elective Subjects</Title>
        <Group gap="sm">
              ...
              {records.length === 0 ? (
                <Table.Tr>
                  <Table.Td colSpan={cols.length + 1}>
                    <Center py="xl"><Text c="dimmed">No elective subjects records found</Text></Center>
                  </Table.Td>
                </Table.Tr>
